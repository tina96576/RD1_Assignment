...
//var_dump($current[0]->weatherElement[0]->time[0]->startTime);
...
foreach($current as $value){
    ...
    for($i=0;$i<5;$i++){
        ...
        echo " ";
        echo $value->weatherElement[$i]->time[0]->parameter->parameterName;
        $tmp[$i]=$value->weatherElement[$i]->time[0]->parameter->parameterName;
...
//     $s_time="SELECT time1,locationname FROM onehrian WHERE time=$stime and locationname=$location";
...
//         $get_time=substr($twoday_t, 11, -6); //2020-08-29
//         $get_date=substr($twoday_t, 0, 10);  //09:00:00
 
   
//         $s_time="SELECT city,time1 FROM week WHERE time='$twoday_t'";
//         require("conn.php");
//         $result1=mysqli_query($link,$s_time);
//         $row1=mysqli_fetch_assoc($result1);
